<x-layout>
  ...
  <div class="py-4 px-4 mx-auto max-w-screen-xl lg:py-4 lg:px-0">
    <div class="mx-auto max-w-screen-md sm:text-center mb-10">
      <form action="/posts">
        @if (request('category'))
        ...
        <div class="items-center mx-auto mb-3 space-y-4 max-w-screen-sm sm:flex sm:space-y-0">
          <div class="relative w-full">
            ...
            <input
              ...
              placeholder="Search for article" type="text" id="search" name="search" autofocus
              autocomplete="off">
          </div>
          <div>
            <button type="submit"
    ...
    @if (count($posts) == 0)
      <div class="text-center py-8">
        <p class="font-semibold text-xl mb-4">Article not found!</p>
        <a href="/posts" class="bg-blue-500 text-white py-2.5 px-4 rounded-lg">&laquo; Back to all post</a>
      </div>
    @else
        ...
        @foreach ($posts as $post)
          <article
            ...
            <a href="/posts/{{ $post->slug }}">
              <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white hover:text-primary-900">
                {{ $post->title }}</h2>
            </a>
